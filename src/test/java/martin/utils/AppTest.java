...
//				System.out.println("drill #: " + i + ", averageDistance: " + averageDistancePerMinute);
	...
	public void threeDice() {
		...
		int numberOfRolls = 100000000;
		int[] totalRolled = new int[19];
		for (int i = 0; i < numberOfRolls; i++) {
			int diceRolls = random.nextInt(5) + 2 + random.nextInt(5) + 2 + random.nextInt(5) + 2;
			totalRolled[diceRolls]++;
		}
		System.out.println("----------Three dice----------");
		double totalSoFar = 0;
		for (int i = 6; i < totalRolled.length; i++) {
			totalSoFar += totalRolled[i];
			System.out.println("Number rolled: " + i + ", " + totalRolled[i] * 100.0 / numberOfRolls + "%, cumulative: " + totalSoFar / numberOfRolls * 100);
		}
	}
	...
	public void fourDice() {
		...
		int[] totalRolled = new int[19];
		for (int i = 0; i < numberOfRolls; i++) {
			totalRolled[diceTotal()]++;
		}
		System.out.println("----------Four dice----------");
		double totalSoFar = 0;
		for (int i = 3; i < totalRolled.length; i++) {
			totalSoFar += totalRolled[i];
			System.out.println("Number rolled: " + i + ", " + totalRolled[i] * 100.0 / numberOfRolls + "%, cumulative: " + totalSoFar / numberOfRolls * 100);
		}
	}
